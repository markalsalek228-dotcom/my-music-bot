...
async def start(client, message):
    await message.reply_text("أهلاً بك! أرسل رابط يوتيوب وسأقوم بتحويله لصوت. 🎵")
...
async def play_music(client, message):
    ...
    sent_msg = await message.reply_text("جاري التحميل... يرجى الانتظار ثواني ⏳")
    
    try:
        # إعدادات التحميل بدون الحاجة لـ FFmpeg خارجي
        ydl_opts = {
        ...
        await sent_msg.edit(f"حدث خطأ: {str(e)[:50]}")
